...
class PlanetAdapterItem(val item: Planet, val clickListener: ClickListener): AdapterItem {
  ...
  override fun bindData(viewHolder: RecyclerView.ViewHolder) {
    ...
    viewHolder.firstTextItem.text = item.terrain
    viewHolder.secondTextItem.text = item.climate
    viewHolder.leftImageView.setImageResource(R.drawable.planet)
    viewHolder.rightImageView.setImageResource(R.drawable.planet)
